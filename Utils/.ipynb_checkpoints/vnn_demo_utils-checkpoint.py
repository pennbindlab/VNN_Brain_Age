...
def FTDC_transfer(Training_set, Test_set):

    res_train = int(Training_set)
    res_test = int(Test_set)
    
    X = torch.load('Data/CT_{}_CN.pt'.format(Test_set)) # z-score normalized cortical thickness 
    C = torch.load('Data/C_{}_CN.pt'.format(Test_set)) # anatomical covariance matrix (normalized such that max eigenvalue is 1)
    
    xAll = torch.tensor(np.expand_dims(X, axis=1))

    yhat_all = torch.zeros((X.shape[1],1))
    
    perms = 100 # There are 100 nominal VNN models


    for i in range(perms):
        with torch.no_grad():

            if res_train == 300: # if selected VNNs were trained on FTDC300
                GNN1Ly = VNNarchit.SelectionGNN([1,39,39], [3,2], True, nn.ReLU, [300,300], 
                                    gml.NoPool, [1,1], [1],torch.zeros((300,300)) )
                GNN2Ly =  VNNarchit.SelectionGNN([1,39,39], [3,2], True, nn.ReLU, [res_test,res_test],  
                                     gml.NoPool, [1,1], [1],C )

            if res_train == 100: # if selected VNNs were trained on FTDC100
                GNN1Ly = VNNarchit.SelectionGNN([1,26,26], [2,2], True, nn.ReLU, [100,100], 
                                    gml.NoPool, [1,1], [1],torch.zeros((100,100)) )
                GNN2Ly =  VNNarchit.SelectionGNN([1,26,26], [2,2], True, nn.ReLU, [res_test,res_test], 
                                 gml.NoPool, [1,1], [1],C )

            if res_train == 500: # if selected VNNs were trained on FTDC500
                GNN1Ly = VNNarchit.SelectionGNN([1,27,27], [4,2], True, nn.ReLU, [500,500], 
                                    gml.NoPool, [1,1], [1],torch.zeros((100,100)) )
                GNN2Ly =  VNNarchit.SelectionGNN([1,27,27], [4,2], True, nn.ReLU, [res_test,res_test], 
                                 gml.NoPool, [1,1], [1],C )

        
            GNN1Ly.load_state_dict(torch.load('FTDC_{}x7_models/test{}.pth'.format(res_train,i))) # load VNN model i 
            GNN2Ly.GFL.load_state_dict(GNN1Ly.GFL.state_dict()) # transfer VNN model i
            yhat_all = yhat_all + GNN2Ly(xAll[:,:,:].T) # Output of VNN model i after transferring


    yhat_all = yhat_all / perms # Mean of outputs obtained from all VNNs
...
def reg_profile_oasis(training_set):
    ...
    # Training set: The sdataset on which the selected VNNs were trained ('148' for OASIS-3, '100' for FTDC100, '300' for FTDC300, '500' for FTDC500)
    
    df_OASIS = pd.read_excel('Data/CT_OASIS.xlsx')
    ...
    for i in range(perms):
        with torch.no_grad():
            ...
            if VNN == 100: # if selected VNNs were trained on FTDC100
                GNN1Ly = VNNarchit.SelectionGNN([1,26,26], [2,2], True, nn.ReLU, [100,100], 
                                      gml.NoPool, [1,1], [1],torch.zeros((100,100)) )
                GNN2Ly =  VNNarchit.SelectionGNN([1,26,26], [2,2], True, nn.ReLU, [res,res], 
                                     gml.NoPool, [1,1], [1],C )
                fbank = 26
            if VNN  == 500: #if selected VNNs were trained on FTDC500
                GNN1Ly = VNNarchit.SelectionGNN([1,27,27], [4,2], True, nn.ReLU, [500,500], 
                                    gml.NoPool, [1,1], [1],torch.zeros((500,500)) )
                GNN2Ly =  VNNarchit.SelectionGNN([1,27,27], [4,2], True, nn.ReLU, [res,res], 
                                     gml.NoPool, [1,1], [1],C )
                fbank = 27
            if VNN == 300: #if selected VNNs were trained on FTDC100
                GNN1Ly = VNNarchit.SelectionGNN([1,39,39], [3,2], True, nn.ReLU, [300,300], 
                            gml.NoPool, [1,1], [1],  torch.zeros((300,300)) )
                GNN2Ly =  VNNarchit.SelectionGNN([1,39,39],[3,2], True, nn.ReLU, [res,res], 
                                     gml.NoPool, [1,1], [1],C )
                fbank = 39
            ...
            if VNN !=148:
                       GNN1Ly.load_state_dict(torch.load('FTDC_{}x7_models/test{}.pth'.format(VNN,i))) 
            else:
                       GNN1Ly.load_state_dict(torch.load('OASIS3_VNN/test{}.pth'.format(i)))
